...
input_args.dz = [0.06, 2.54, 1000]; % multi layer column 
...
input_args.rho_snw(1:nbr_lyr) = [1000, 911.5, 1000]; 
...
out_fl = 'mosaic1_saw.txt';
%writematrix(di.albedo, out_fl)



input_args.rho_snw(1:nbr_lyr) = [1000, 912.5, 1000];
use_ssl = 1;
if (use_ssl == 1)
    nbr_lyr = nbr_lyr+1;
end
...
str = {'\theta = 40^{\circ}','{\itdz}_{w} = 0.06 m','{\itdz}_{i} = 2.54 m','\rho_{ice}= 911.5 kg/m^3'};
dim = [0.65 0.37 0.29 0.4];
if (1==1)
    figure(2)
    hold on
    plot(mosaic_wvl, mosaic_var1, 'linewidth',3, 'DisplayName','MOSAiC Site 1', 'color','k');
    plot(di.wvl,di.albedo,'linewidth',3, 'DisplayName','SNICAR4', 'color','red');
    plot(di.wvl,ds.albedo,'linewidth',3, 'DisplayName','SNICAR4 SSL','color','b');
    axis([0.2 2.6 0 1]);
    set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    set(gca,'ytick',0:0.1:1.0,'fontsize',14);
