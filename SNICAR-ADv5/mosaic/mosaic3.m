...
input_args.dz = [0.13, 1.12, 1000]; % multi layer column 
...
input_args.rho_snw(1:nbr_lyr) = [1000, 914.8, 1000]; 
...
input_args.chl_wtr(1:nbr_lyr)               = 0.0; % [mg/m^3]
...
out_fl = 'mosaic3_saw.txt';
%writematrix(di.albedo, out_fl)

input_args.rho_snw(1:nbr_lyr) = [1000, 916.999, 1000]; 
use_ssl = 1;
if (use_ssl == 1)
    nbr_lyr = nbr_lyr+1;
end
...
ds = snicarAD_v4_ssl(input_args);




mosaic_wvl  = readmatrix('mosaic_data.xlsx', 'Range','A11:A2015');
...
%str = {'SZA = 40^{\circ}','wtr\_thk = 0.13 m','ice\_thk = 1.12 m','\rho\_ice=914.8 kg/m^3'};
%dim = [0.62 0.37 0.29 0.4];
str = {'\theta = 40^{\circ}','{\itdz}_{w} = 0.13 m','{\itdz}_{i} = 1.12 m','\rho_{ice}= 914.8 kg/m^3'};
dim = [0.65 0.37 0.29 0.4];
if (1==1)
    figure(2)
    hold on
    plot(mosaic_wvl, mosaic_var3, 'linewidth',3, 'DisplayName','MOSAiC Site 2', 'color','k');
    plot(di.wvl,di.albedo,'linewidth',3, 'DisplayName','SNICAR-ADv5', 'color','red');
    plot(di.wvl,ds.albedo,'linewidth',3, 'DisplayName','SNICAR-ADv5 SSL', 'color','b');
    axis([0.2 2.6 0 1]);
    set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    set(gca,'ytick',0:0.1:1.0,'fontsize',14);
    title('Comparison of SNICAR-ADv5 with Observations')
    xlabel('Wavelength (\mum)','fontsize',20);
    ylabel('Hemispheric Albedo','fontsize',20);
    %0.13 - 1.12
