...
str = {'\theta = 73.7^{\circ}','\rho = 916.5 kg/m^3','{\itr}_b = 300 \mum'};
dim = [0.67 0.39 0.29 0.4];
...
if (0==1)
    ...
    legend('fontsize',16);
    hold on
    saveas(gcf,'~/Desktop/ppr1/alb_ice','epsc')
end
...
if (1==0)
    ...
    hold on
    plot(di.wvl,di2.flx_dwn_spc,'linewidth',3, 'DisplayName','Cloudy','color','red');
    %axis([0.2 2.6 0 1]);
    %set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    %set(gca,'ytick',0:0.1:1.0,'fontsize',14);
    title('Sub-Arctic Summer Insolation', 'fontsize',20)
    xlabel('Wavelength (\mum)','fontsize',20);
    ylabel('Insolation [W m^{-2}]','fontsize',20);
    %text(1.9,0.8,str, 'fontsize',16)
    %annotation('textbox',dim,'String',str,'FitBoxToText','on', 'Fontsize',14);
    grid on;
    legend('fontsize',16);
