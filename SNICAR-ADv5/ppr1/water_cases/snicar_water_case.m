...
input_args.chl_wtr = [3.0]; % [mg/m^3]

% CALL SNICAR WITH INPUT ARGUMENTS
di = snicarAD_v4(input_args);

str = {'\theta = 52.8^{\circ}','dz_w = 1000 m','{[Chl]} = 3.0 mg/m^3'};
%dim = [0.65 0.39 0.29 0.4];
dim = [0.65 0.39 0.29 0.4];
...
if (1==0)
    figure(1)
    plot(di.wvl,di.albedo,'linewidth',3, 'DisplayName','Direct');
    hold on
    plot(di.wvl,di2.albedo,'linewidth',3, 'DisplayName','Diffuse', 'color','red');
    hold on
    %plot(di.wvl,di3.albedo,'linewidth',3, 'DisplayName','Direct (No Chlorophyll)', 'Color', grayColor, 'linestyle',':');
    
    axis([0.2 2.6 0 0.1501]);
    set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    set(gca,'ytick',0:0.05:1.0,'fontsize',14);
    title('Albedo of Water', 'fontsize',20)
    ...
    legend('fontsize',16);
    hold on 
    saveas(gcf,'~/Desktop/ppr1/alb_wtr','epsc')
end
...
if (0==1)
    ...
    title('Tropical Summer Insolation', 'fontsize',20)
    xlabel('Wavelength (\mum)','fontsize',20);
    ylabel('Fraction Insolation','fontsize',20);
    %text(1.9,0.8,str, 'fontsize',16)
    %annotation('textbox',dim,'String',str,'FitBoxToText','on', 'Fontsize',14);
    grid on;
    legend('fontsize',16);
    hold on 
    saveas(gcf,'~/Desktop/ppr1/trp_insolation','epsc')
end


[spc, brd] = binning(di);
spc_path = '/Users/jtolento/TZW23/RRTMG_SW/run_examples_std_atm/ppr1/clr/input_trp_spc';  % Replace with the actual file path
brd_path = '/Users/jtolento/TZW23/RRTMG_SW/run_examples_std_atm/ppr1/clr/input_trp_brd';
...
[spc, brd] = binning(di);
spc_path = '/Users/jtolento/TZW23/RRTMG_SW/run_examples_std_atm/ppr1/cld/input_trp_spc';  % Replace with the actual file path
brd_path = '/Users/jtolento/TZW23/RRTMG_SW/run_examples_std_atm/ppr1/cld/input_trp_brd';
binned_albedo = spc;  % Replace with your binned albedo values
line = 7;          %7 for direct, 8 for diffuse
modify_rrtm_input(spc_path, spc,line);
modify_rrtm_input(brd_path, brd,line)
