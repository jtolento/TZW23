...
sza = 60;
wtr_thk = logspace(-3.0, 0.0, 5);
nbr_wtr = length(wtr_thk);
for idx = 1: nbr_wtr
    idx = idx
    input_args.coszen = cos(deg2rad(sza));
  
    % SNOW LAYER THICKNESSES [m]:
    input_args.dz = [wtr_thk(idx), 3, 1000]; % multi layer column 
    ...
    input_args.chl_wtr = [0.0, 0.0, 0.0]; % [mg/m^3]

    % CALL SNICAR WITH INPUT ARGUMENTS
    di = snicarAD_v4(input_args);
    % plot modeled spectral albedo:
    thickness = num2str(wtr_thk(idx), '%3.3f');
    thickness = append(thickness,' m');
    str = {'\theta = 60^{\circ}','{\itdz}_{i} = 3 m','\rho_{ice}= 900 kg/m^3'};
    dim = [0.68 0.37 0.29 0.33]; 
    if (1==1)
       figure(1)
       plot(di.wvl,di.albedo,'linewidth',3, 'DisplayName', thickness);
       axis([0.2 2.6 0 1]);
       set(gca,'xtick',0.2:0.5:5,'fontsize',14)
       set(gca,'ytick',0:0.1:1.0,'fontsize',14);
       title('Varying Pond Thickness over Ice', 'fontsize',20)
...
%%%
    ...
    %%%% -- Output albedo file to be 
    if(input_args.atm == 1)
       atm_typ = 'MLW';
    elseif(input_args.atm == 2)
         atm_typ = 'MLS';
     elseif(input_args.atm == 3) 
        atm_typ = 'SAW';
     elseif(input_args.atm == 4)
        atm_typ = 'SAS';  
    end

    if(input_args.direct_beam == 1)
        cld_cvr = 'clr';
    elseif(input_args.direct_beam == 0)
       cld_cvr = 'cld';
    end
    %sza = num2str(sza);
    
    if input_args.lyr_typ(1) == 1
        surf_typ = 'snw';
    elseif input_args.lyr_typ(1) == 2
        surf_typ = 'ice';
    elseif input_args.lyr_typ(1) == 3
        surf_typ = 'wtr';   
    end
    %str = 'alb';
    %out_fl = append(str, '_', surf_typ, '_', atm_typ, '_', cld_cvr, '_', sza, '.txt');
    %writematrix(di.albedo, out_fl)
...
if (0==1)
    figure(1)
    plot(sza,alb_vis,'linewidth',3, 'DisplayName','500 nm');
    axis([0 90 0 1]);
    %set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    %set(gca,'ytick',0:0.1:1.0,'fontsize',14);
    xlabel('Solar Zenith Angle (Degrees)','fontsize',20);
    ylabel('Hemispheric Albedo','fontsize',20);
    title('Albedo of Semi-Infinite Layer of Water');
    grid on;
    legend();
    hold on 
end
...
%%% 
%mosaic_wvl = mosaic_wvl.*(10^(-3));
%mosaic_var1 = readmatrix('mosaic_data.xlsx', 'Range','B11:B2015');
%mosaic_var2 = readmatrix('mosaic_data.xlsx', 'Range','C11:C2015');
%mosaic_var3 = readmatrix('mosaic_data.xlsx', 'Range','D11:D2015');
if (0==1)
    figure(2)
    hold on
    plot(mosaic_wvl, mosaic_var1, 'linewidth',3, 'DisplayName','0.06 - 2.54');
    plot(mosaic_wvl, mosaic_var2, 'linewidth',3, 'DisplayName','0.09 - 1.12');
    plot(mosaic_wvl, mosaic_var3, 'linewidth',3, 'DisplayName','0.13 - 1.12');
    axis([0.2 2.6 0 1]);
    set(gca,'xtick',0.2:0.5:5,'fontsize',14)
    set(gca,'ytick',0:0.1:1.0,'fontsize',14);
    xlabel('Wavelength (\mum)','fontsize',20);
